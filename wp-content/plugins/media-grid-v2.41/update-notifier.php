...
define( 'MG_NOTIFIER_PLUGIN_FILE_NAME', 'media-grid.php' ); // The plugin file name
define( 'MG_NOTIFIER_PLUGIN_XML_FILE', 'www.projects.lcweb.it/wp-content/plugins/media-grid/notifier.xml' ); // The remote notifier XML file 
define( 'MG_PLUGIN_NOTIFIER_CACHE_INTERVAL', 600 ); // The time interval for the remote XML cache in the database (21600 seconds = 6 hours)
define( 'MG_PLUGIN_NOTIFIER_CODECANYON_USERNAME', 'LCweb' ); // Your Codecanyon username
...
function mg_update_notifier_bar_menu() {
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		if( (string)$xml->latest > (string)$plugin_data['Version']) { // Compare current plugin version with the remote XML version
			$wp_admin_bar->add_menu( array( 'id' => 'plugin_update_notifier', 'title' => '<span>' . MG_NOTIFIER_PLUGIN_NAME . ' <span id="ab-updates">New Updates</span></span>', 'href' => get_admin_url() . 'index.php?page=mg-plugin-update-notifier' ) );
		}
	}
}
add_action( 'admin_bar_menu', 'mg_update_notifier_bar_menu', 1000 );
