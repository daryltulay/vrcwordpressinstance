...
function mg_register_metaboxes() {
	add_meta_box('mg_item_size_box', __('Item Sizes (optional)', 'mg_ml'), 'mg_item_size_box', 'mg_items', 'side', 'low');
...
function mg_item_size_box() {
    ...
    <div class="lcwp_sidebox_meta">
        <div class="misc-pub-section">
          <label><?php _e('Item Width', 'mg_ml') ?></label>
          <select data-placeholder="<?php _e('Select a size', 'mg_ml') ?> .." name="mg_width" class="chzn-select" tabindex="2">
            <?php 
			foreach($vals as $val) {
				($val == $width) ? $sel = 'selected="selected"' : $sel = '';
				echo '<option value="'.$val.'" '.$sel.'>'. str_replace('_', '/', $val) .' '. __('of the container width', 'mg_ml').'</option>'; 
			}
			?>
          </select> 
        </div>
        ...
        <div class="misc-pub-section-last">
          <label><?php _e('Item Height', 'mg_ml') ?></label>
          <select data-placeholder="<?php _e('Select a size', 'mg_ml') ?> .." name="mg_height" class="chzn-select" tabindex="2">
            <?php 
			$vals[] = 'auto'; // add height == auto
			foreach($vals as $val) {
				$txt = ($val == 'auto') ? __('Adjust automatically', 'mg_ml') : str_replace('_', '/', $val) .' '. __('of the container width', 'mg_ml');
...
function mg_item_opt_box() {
    ...
    <div class="lcwp_mainbox_meta">
      <table class="widefat lcwp_table lcwp_metabox_table">
        <tr>
          ...
          <td class="lcwp_field_td">
              <select data-placeholder="<?php _e('Select a size', 'mg_ml') ?> .." name="mg_main_type" id="mg_main_type" class="chzn-select" tabindex="2">
                <?php 
	  ...
	  <?php if(!$main_type || $main_type=='simple_img' || $main_type=='link' || $main_type=='lb_text' || $main_type=='spacer') echo 'style="display: none;"' ?>>
        <table class="widefat lcwp_table lcwp_metabox_table">
          <tr>
            <td class="lcwp_label_td"><?php _e("Item Layout", 'mg_ml'); ?></td>
            <td class="lcwp_field_td">
                <select data-placeholder="<?php _e('Select a layout', 'mg_ml') ?> .." name="mg_layout" id="mg_layout" class="chzn-select" tabindex="2">
            ...
            <td><span class="info"><?php _e("Set the item layout when it's active", 'mg_ml') ?></span></td>
          </tr>
          <tr>
	  ...
	  <?php if(!$main_type || ($main_type!='single_img' && $main_type!='img_gallery' && $main_type!='audio')) echo 'style="display: none;"' ?>>
        ...
        <table class="widefat lcwp_table lcwp_metabox_table">
          ...
          <tr>
            <td class="lcwp_label_td"><?php _e("Full image resize method", 'mg_ml'); ?></td>
            <td class="lcwp_field_td">
                <select data-placeholder="<?php _e('Select a method', 'mg_ml') ?> .." name="mg_img_res_method" id="mg_img_res_method" class="chzn-select" tabindex="2">
                  <option value="1" <?php if($resize_method == '1') echo 'selected="selected"';?>><?php _e('Resize and crop', 'mg_ml') ?></option>
                  <option value="2" <?php if($resize_method == '2') echo 'selected="selected"';?>><?php _e('Standard resize', 'mg_ml') ?></option>
                </select>
            </td>     
            <td><span class="info"><?php _e('Set the resize method for the full size image', 'mg_ml') ?></span></td>
          </tr>
        </table>  
      </div>
     ...
     <?php // USER CUSTOM OPTIONS ////////// ?> 
      ...
      <div id="mg_builder_wrap">
          ...
          <div id="mg_builder_img_gallery" <?php if(!$main_type || $main_type != 'img_gallery') {echo 'style="display: none;"';} ?>>	
            ...
            <table class="widefat lcwp_table lcwp_metabox_table" style="margin-bottom: 0px;">
              <tr>
                ...
                <td class="lcwp_field_td">
                    ...
                    <select name="mg_slider_w_type" style="width: 50px; margin-left: -5px;">
                      <option value="%">%</option>
                      <option value="px" <?php if($h_type == 'px') {echo 'selected="selected"';} ?>>px</option>
                    </select>  
                </td>
                <td><span class="info"><?php _e("Slider height (% is related to the width)", 'mg_ml'); ?></span></td>
              </tr>
            </table>
            ...
            <h4>Choose the images <span class="mg_TB mg_upload_img add-new-h2"><?php _e('Manage Images', 'mg_ml') ?></span></h4>
            <div id="gallery_img_picker"></div>	
          </div>
          
          <?php // video builder ?>
          <div id="mg_builder_video" class="lcwp_form" <?php if(!$main_type || $main_type != 'video') {echo 'style="display: none;"';} ?>>
              <h4>Video Options</h4>
              <table class="widefat lcwp_table lcwp_metabox_table">
                <tr>
                  ...
                  <td colspan="2">
					...
					<?php if((float)substr(get_bloginfo('version'), 0, 3) >= 3.6): ?>
                    	<img src="<?php echo MG_URL ?>/img/media-library-src.png" alt="(SRC)" title="search in media library" style="margin: 0 0 -6px 1px; cursor: pointer;" id="mg_video_src" />
					<?php endif; ?><br/>
                    
                    <span class="info"><?php _e('Insert the Youtube (<strong>http://youtu.be</strong>) or Vimeo clean video url', 'mg_ml') ?> 
					<?php if((float)substr(get_bloginfo('version'), 0, 3) >= 3.6) { _e('Otherwise select a video from your media library', 'mg_ml'); }; ?></span>
                  </td>
                </tr>
                ...
                <tr>
                  ...
                  <td>
                      <span class="info"><?php _e('If checked, set the featured image as poster (for self hosted video)', 'mg_ml') ?></span>
                  </td>
                </tr>
                <?php endif; ?>
              </table>
          ...
          <div id="mg_builder_audio" <?php if(!$main_type || $main_type != 'audio') {echo 'style="display: none;"';} ?>>
			 <?php $tracks = mg_existing_sel(get_post_meta($post->ID, 'mg_audio_tracks', true)); ?>
              ...
              <h4><?php _e('Choose the tracks', 'mg_ml') ?> <span class="mg_TB mg_upload_audio add-new-h2"><?php _e('Manage Tracks', 'mg_ml') ?></span></h4>
...
function mg_item_featured_image(){
	...
	if ($current_screen->id == 'mg_items' && $current_screen->parent_base == 'edit') {
		...
		if($main_type && $main_type != 'spacer' && get_the_post_thumbnail($post->ID) == '') {
